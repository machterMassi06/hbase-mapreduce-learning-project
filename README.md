...
Tu peux écrire cette section de manière plus propre et plus professionnelle :

### 2. Compute the Number of Visits per Country (Classic MR)

> Using a classic MapReduce job.
> Source code: `mapreduce/src/main/java/mapreduce/nb_visits_by_country`
...
### 3. Compute the Number of Visits per Aggregation Column (Table MR)

> Using a generic Table MapReduce job that reads data directly from HBase.
> Source code: `mapreduce/src/main/java/mapreduce/nb_visits_by_column_TableMR`

This job reads records directly from the HBase table `web_site.visits` and computes the number of visits grouped by a specified column.
...
### Results Verification (Example: Visits per Country)

Display the results generated by the classic MapReduce job:

```bash
hdfs dfs -cat /nb_visits_per_country_output/part-r-00000
```

Display the results generated by the Table MapReduce job:

```bash
hdfs dfs -cat /nb_visits_per_country_TableMR_output/part-r-00000
```

Both jobs should produce identical results:
...
This confirms that the Table MapReduce implementation correctly reads data directly from HBase and produces the same aggregation results as the classic MapReduce implementation based on CSV files.

---
# TODO
* Run MapReduce jobs from hbase tables (join)
